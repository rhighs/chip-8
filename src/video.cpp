...
Video::Video(std::string title, int winw, int winh, int texw, int texh){
    SDL_Init(SDL_INIT_VIDEO);
    w = SDL_CreateWindow(title.c_str(), 0, 0, winw, winh, SDL_WINDOW_SHOWN);
    rdr = SDL_CreateRenderer(w, -1, SDL_RENDERER_ACCELERATED);
    tex = SDL_CreateTexture(rdr, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_STREAMING, texw, texh);
}
...
void Video::update(void const* buffer, int pitch){
    SDL_UpdateTexture(tex, nullptr, buffer, pitch);
    SDL_RenderClear(rdr);
    SDL_RenderCopy(rdr, tex, nullptr, nullptr);
    SDL_RenderPresent(rdr);
}

bool Video::process_input(uint8_t* keys){
    bool quit = false;
    SDL_Event event;
